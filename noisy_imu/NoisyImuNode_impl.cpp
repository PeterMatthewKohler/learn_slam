...
{
    // Parameters: stddevs for each component group
...
void NoisyImuNode::imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg)
{
    ...
    // Update orientation covariance if available (not sentinel -1)
    ...
    // Update angular velocity covariance if available (not sentinel -1)
    ...
    // Update linear acceleration covariance if available (not sentinel -1)
    ...
    noisy.header.frame_id = msg->header.frame_id; // preserve frame
