        ...
        if(first_visual_frame || tracked_features_.empty()) {
            ...
            if(!pose){
                ...
                if(visualPoseChain_->stamp.get_clock_type() !=
                   ...
                   stamp <= visualPoseChain_->stamp) {
                    ...
                    const auto composed =
                        composeVisualCameraPose(
                            visualPoseChain_->camera_pose,
                            *pose
                        );
                    if(!composed){
                        const bool chain_advanced =
            ...
            if(should_add_new) {
                const int num_to_add =
                    tracked_features_.size() < static_cast<std::size_t>(max_features)
                        ? max_features - static_cast<int>(tracked_features_.size())
                        : 0;
                if(num_to_add > 0) {
